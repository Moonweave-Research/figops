...
def _manifest(*, visual_passed: bool = True, include_layout: bool = True) -> dict[str, object]:
...
def test_readiness_manifest_json_and_revision_exit_code(tmp_path: Path) -> None:
    path = tmp_path / "manifest.json"
    path.write_text(json.dumps(_manifest(visual_passed=False)), encoding="utf-8")
...
def test_readiness_manifest_blocks_when_required_evidence_is_missing(tmp_path: Path) -> None:
    path = tmp_path / "manifest.json"
    path.write_text(json.dumps(_manifest(include_layout=False)), encoding="utf-8")
