...
def test_expected_wheel_name_uses_current_project_version(tmp_path: Path) -> None:
    ...
    assert expected_wheel_name(tmp_path) == "graph_making_hub-1.2.3-py3-none-any.whl"
...
def test_consumer_smoke_commands_use_isolated_uv_with_console_scripts(tmp_path: Path) -> None:
    wheel = tmp_path / "graph_making_hub-1.2.3-py3-none-any.whl"

    commands = consumer_smoke_commands(wheel, uv_bin="uv-test")

    assert commands == (
        ("uv-test", "run", "--isolated", "--with", str(wheel), "graphhub-mcp", "--smoke"),
        ("uv-test", "run", "--isolated", "--with", str(wheel), "graphhub", "--help"),
    )
