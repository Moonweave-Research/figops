# FigOps MCP Error Taxonomy

FigOps MCP uses a small error taxonomy across JSON-RPC protocol errors and
tool `structuredContent` error envelopes. The JSON-RPC `error.code` remains the
protocol-level code. FigOps adds stable taxonomy fields so clients do not
...
| `internal` | `GRAPHHUB_INTERNAL` | `-32603` | An unexpected FigOps, runtime, or handler failure occurred. |
