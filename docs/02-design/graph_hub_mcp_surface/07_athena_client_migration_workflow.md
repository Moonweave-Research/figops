...
- Purpose: separate Graph Hub ownership from Athena orchestration without losing current bridge behavior
...
Athena command / visualization flow
  ...
  -> Graph Hub orchestrator.py
  -> Graph Hub core rendering and artifacts
```

This path works and must remain available while MCP parity is built.

Current Graph Hub-to-Athena path:

```text
Graph Hub hub_core.athena_bridge
  -> imports Athena visualizer engine
  -> renders Athena-style diagrams from Hub specs
```

This path is useful but increases coupling. Treat it as a transitional adapter or plugin candidate, not as the core Graph Hub ownership model.

## Migration Principles

1. Preserve existing Athena bridge behavior until an MCP path proves equivalent output and diagnostics.
2. Move ownership of style, discovery, validation, execution, provenance, and artifacts to Graph Hub.
3. Make Athena depend on Graph Hub through a typed client interface, not by duplicating Graph Hub enums or file layout assumptions.
...
- Athena accepts every Graph Hub official target format.
- A parity test compares Athena's bridge target format set with Graph Hub's `ALLOWED_TARGET_FORMATS`.
- `nature_surfur` is explicitly tested.
- Unsupported values such as `baseline` and `ieee` remain rejected.

### Step 1 - Read-Only Graph Hub MCP
...
Goal: introduce an Athena-side Graph Hub client that can use MCP when available and fall back to the existing bridge when not.
...
Goal: add `graphhub.render_ephemeral` or `graphhub.render_csv_graph` and prove it matches the current bridge.
...
- route normal Athena graph rendering through Graph Hub MCP,
- keep fallback for emergency/local operation until a release checkpoint.

### Step 5 - Review Reverse Imports

Goal: reduce Graph Hub-to-Athena coupling.

Options:

- keep `hub_core.athena_bridge` as an optional plugin adapter,
- move Athena-style diagram rendering into Graph Hub,
- or expose the Athena visualizer as a separate typed tool that Graph Hub calls intentionally.
...
- `nature_surfur` can flow through Athena into Graph Hub.
- Graph Hub read-only MCP calls do not change git status.
