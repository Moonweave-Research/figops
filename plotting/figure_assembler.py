...
def _safe_svg_fromfile(svg_path: str) -> sg.SVGFigure:
    """svgutils.fromfile parses with XMLParser(huge_tree=True) and the default
    resolve_entities=True, so a panel SVG with a DOCTYPE entity bomb (billion laughs)
...
def _read_svg_native_width_mm(svg_path: str) -> float:
    doc = _safe_svg_fromfile(svg_path)
    width_str = doc.width
...
def assemble_figure(fig_id: str, fig_cfg: dict, project_dir: str) -> str:
    ...
    for panel_id, slot in slots.items():
        ...
        if _is_raster(src_path):
            ...
            panel_svg = _safe_svg_fromfile(src_path)
