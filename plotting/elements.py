...
def draw_cantilever_beam(
    ...
    v = np.array([
        [x0, y0, z0], [x0 + L, y0, z0], [x0 + L, y0 + W, z0], [x0, y0 + W, z0],
        [x0, y0, z0 + T], [x0 + L, y0, z0 + T], [x0 + L, y0 + W, z0 + T], [x0, y0 + W, z0 + T]
    ])
...
def draw_electrode_array(
    ...
    for i in range(finger_count):
        ...
        if orientation == 'horizontal':
            ...
            rect_pos = patches.Rectangle((x0, y0 + offset), f_length + bus_bar_width, finger_width, 
                                         color=color_pos, alpha=alpha, ec='black', lw=0.5)
            # Negative finger (Right to Left)
            rect_neg = patches.Rectangle((x0 + width - (f_length + bus_bar_width), y0 + offset + finger_width + finger_gap), 
                                         f_length + bus_bar_width, finger_width, 
                                         color=color_neg, alpha=alpha, ec='black', lw=0.5)
        else: # vertical
            rect_pos = patches.Rectangle((x0 + offset, y0), finger_width, f_length + bus_bar_width, 
                                         color=color_pos, alpha=alpha, ec='black', lw=0.5)
            rect_neg = patches.Rectangle((x0 + offset + finger_width + finger_gap, y0 + height - (f_length + bus_bar_width)), 
                                         finger_width, f_length + bus_bar_width, 
                                         color=color_neg, alpha=alpha, ec='black', lw=0.5)
...
def draw_polymer_network(
    ...
    for _ in range(num_chains):
        ...
        np.random.shuffle(y) # Randomize vertical path
        
        # Add 'wiggle' using sine waves + noise
        y += np.sin(x) * wiggle + np.random.normal(0, wiggle, nodes_per_chain)
...
def draw_zoom_circle(
    ...
    src_circle = patches.Circle(
        center, radius,
        fill=fill, edgecolor=color, linewidth=lw, alpha=alpha, zorder=zorder,
    )
    zoom_circle = patches.Circle(
        zoom_center, zoom_radius,
        fill=fill, edgecolor=color, linewidth=lw, alpha=alpha, zorder=zorder,
    )
    ...
    zx, zy = zoom_center
    line1, = ax.plot(
        [cx, zx], [cy + radius, zy + zoom_radius],
...
def draw_charge_hop(
    ...
    for pos in positions:
        circle = patches.Circle(
            pos, site_radius,
            color=site_color, alpha=alpha, zorder=zorder,
        )
        ax.add_patch(circle)
