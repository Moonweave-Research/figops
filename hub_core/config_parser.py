...
def validate_config(config):
    ...
    if sweep is not None and comparison is not None:
        ...
        if sweep_enabled and comparison_enabled:
            errors.append(
                "sweep.enabled and comparison.enabled cannot both be true; they are mutually exclusive."
            )
            ...
            for fig_id, fig_cfg in assemblies.items():
                ...
                if not isinstance(layout, str) or not layout.strip():
                    ...
                    if rows:
                        ...
                        for ri, row in enumerate(rows):
                            if len(row) != row_len:
                                errors.append(
                                    f"assemblies.{fig_id}.layout row {ri+1} has "
                                    f"{len(row)} cols, expected {row_len}."
                                )
                ...
                if not isinstance(panels, dict):
                    ...
                    for pid, p_cfg in panels.items():
                        ...
                        if 'source' not in p_cfg or not isinstance(p_cfg['source'], str):
                            ...
                            if os.path.isabs(src):
                                errors.append(
                                    f"assemblies.{fig_id}.panels.{pid}.source: "
                    ...
                    if isinstance(layout, str) and layout.strip():
                        ...
                        if extra_panels:
                            errors.append(
                                f"assemblies.{fig_id}: panels {sorted(extra_panels)} "
                                f"are not referenced in layout."
                            )
